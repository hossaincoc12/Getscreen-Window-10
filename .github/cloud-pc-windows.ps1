...
$script:ScratchFiles = @($script:ShotName, $script:DiagName,
...
# --- 1b. does the screen server actually show the desktop? ---------------
# The screen server mirrors the console session, and it can start before that
# session exists - in which case it serves one flat colour for ever, and every
# viewer shows a black screen. (The desktop itself is always there: the runner
# runs inside that same session, with explorer.exe already started.) So the
# picture is looked at here, and the screen server is restarted until it really
# does show the desktop. If the service will not come good, the screen server is
# started in application mode instead, which serves the session directly.
# vncdotool talks to TightVNC exactly the way a viewer does, so the machine can
# check its own screen - and take a picture of it - without a browser in the loop.
...
try:
    ...
    time.sleep(3)
    if target:
        client.captureScreen(target)
    image = client.screen
    w, h = image.size
    colours = image.getcolors(maxcolors=2000000)
    if colours is None:
        print('cloud pc: the screen is', w, 'x', h, 'and is full of colours')
    else:
        colours.sort(reverse=True)
        print('cloud pc: the screen is', w, 'x', h, 'with', len(colours), 'colours; the commonest is', colours[0][1], 'covering', colours[0][0], 'of', w * h, 'pixels')
        print('cloud pc:', 'FLAT' if len(colours) <= 8 else 'RICH')
    client.disconnect()
except Exception:
    print('cloud pc: FLAT')
    traceback.print_exc()
...
for ($try = 1; $try -le 4; $try++) {
  $check = (python $pyPath $shotPath 2>&1 | Out-String)
  Write-Host $check
  if ($check -match "cloud pc: RICH") {
    Note "the screen server is showing the desktop (attempt $try)"
    break
  }
  if ($try -eq 4) { Warn "the screen still shows nothing after 4 tries - the viewer will probably show a blank desktop"; break }
  Note "the screen is blank - restarting the screen server (attempt $try)"
  try { Restart-Service -Name "tvnserver" -Force -ErrorAction Stop }
  catch {
    Note "the screen service would not restart ($($_.Exception.Message)) - running the screen server in application mode instead"
    Get-Process tvnserver -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
